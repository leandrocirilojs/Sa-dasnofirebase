...
function showToast(titulo, mensagem, tipo = 'info', duracao = 4000) {
  if (typeof window.toast === 'function') {
    ...
    console.warn(`[Toast] ${titulo}: ${mensagem}`);
  }
}
...
function setupApp() {
  ...
  expenseForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      showToast('Erro ao Salvar', erro.message, 'error', 6000);
    } finally {
  ...
  window.removeExpense = async (id) => {
    ...
    if (password === "2702") {
      try {
        ...
        showToast('Removido', 'Saída excluída com sucesso.', 'info');
      } catch (erro) {
...
function carregarSaidas(startDate, endDate, driverFilter, storeFilter) {
  ...
  unsubscribe = onSnapshot(q, (snapshot) => {
    ...
    showToast('Erro de Conexão', 'Não foi possível carregar as saídas.', 'error', 6000);
  });
}
...
function gerarPDF() {
  if (!filteredExpenses.length) {
    showToast('PDF Vazio', 'Nenhuma saída filtrada para gerar o PDF.', 'warning');
    return;
  }
...
function exportToExcel() {
  if (!filteredExpenses.length) {
    showToast('Excel Vazio', 'Nenhuma saída filtrada para exportar.', 'warning');
    return;
  }
...
function enviarWhatsApp() {
  if (filteredExpenses.length === 0) {
    showToast('Sem Dados', 'Nenhuma saída filtrada para enviar.', 'warning');
    return;
  }
  ...
  showToast('Relatório', 'Abrindo WhatsApp...', 'success');
}
